...
class Actor(nn.Module):
    ...
    def forward(self, s, deterministic=False, return_logpi=False):
        ...
        if not return_logpi:
            ...
            a = self.action_activation(a)
            logpi -= torch.sum(torch.log(torch.clamp(1 -
                                                     a**2, 0, 1) + 1e-6), dim=-1)
            a = a * self.action_scale + self.action_shift
            return a, logpi
...
class SACAgent(RL.Agent):
    def __init__(self, name, algo, convs, hidden_layers, train_freq, mb_size, gamma, nsteps, td_clip, grad_clip, lr, a_lr, eval_mode, no_train_for_steps, exp_buffer: ExperienceBuffer, alpha=0.2):
