...
class ModelLoadAgent(RL.Agent):
    ...
    def next_model_stepId_epId(self):
        all_files = os.listdir(self.checkpoints_folder)
        # print(self.consumed_paths)
        new_files = set(all_files) - self.consumed_paths
        # print(new_files)
                ...
                for f in new_files:
                    if f.endswith('.model') and f.startswith('step'):
                        ...
                        if step_id > max_step_id:
                            max_step_id = step_id
                            max_ep_id = ep_id
                self.consumed_paths.update(all_files)
    ...
    def update_model(self):
        while True:
            ...
            if path is not None:
                ...
                self.loaded_one_model = True
                print(self.loaded_one_model)
                break
            else:
                # print(self.wait_for_new, self.loaded_one_model)
                # if self.wait_for_new or not self.loaded_one_model:
